{{-- Main header row: logo + search form + right actions (user, cart) --}}
        ...
        <div class="cs_main_header_in">
            <div class="cs_main_header_left">
                ...
                <div class="cs_header_search_form_wrap">
                    <form action="{{ route('products.index') }}" method="GET" class="cs_header_search_with_category_form">
                        <input type="text" name="search" placeholder="Search for medicines, vitamins…"
                               value="{{ request('search') }}"
                               class="cs_header_search_with_category_field">

                        <div class="cs_header_category_wrap cs_light cs_dropdown">
                            <button class="cs_header_category_toggle cs_dropdown_btn" type="button">
                                <span>All Categories</span>
                                <svg width="10" height="5" viewBox="0 0 10 5" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M5.00002 5C4.88377 5 4.76741 4.95927 4.67866 4.87792L0.13321 0.711272C-0.0444033 0.54846 -0.0444033 0.284816 0.13321 0.122109C0.310823 -0.0405988 0.598436 -0.0407029 0.775936 0.122109L5.00002 3.99417L9.22411 0.122109C9.40172 -0.0407029 9.68933 -0.0407029 9.86683 0.122109C10.0443 0.28492 10.0444 0.548565 9.86683 0.711272L5.32138 4.87792C5.23263 4.95927 5.11627 5 5.00002 5Z" fill="#303030"/>
                                </svg>
                            </button>
            ...
            <div class="cs_main_header_right">
                <button class="cs_mobile_search_toggle cs_mobile_show" type="button" aria-label="Toggle search">
                    <img src="{{ asset('assets/glowify/images/icons/search.svg') }}" alt="Search">
                </button>
                ...
                <ul class="cs_header_list cs_mp_0">
                    <li>
                        <div class="cs_header_credential_btns">
                            @auth
                                <a href="{{ route('dashboard') }}" title="My Account"><img src="{{ asset('assets/glowify/images/icons/user.svg') }}" alt="Account"></a>
                                <span>
                                    Hi, <a href="{{ route('dashboard') }}">{{ \Illuminate\Support\Str::words(auth()->user()->name, 1, '') }}</a>
                                </span>
                            @else
                                <a href="{{ route('login') }}" title="Log in"><img src="{{ asset('assets/glowify/images/icons/user.svg') }}" alt="User"></a>
                                <span>
                    ...
                    <li>
                        <button type="button" class="cs_cart_btn" data-bs-toggle="offcanvas" data-bs-target="#mnCartCanvas" aria-label="Open cart"
                                x-data>
                            <img src="{{ asset('assets/glowify/images/icons/bag.svg') }}" alt="Cart">
                            <span class="cs_cart_label cs_accent_bg cs_white_color"
                                  x-text="$store.cart.count > 99 ? '99+' : $store.cart.count"
                                  x-show="$store.cart.count > 0" x-cloak>0</span>
                            <span>My Bag</span>
                        </button>
                    </li>
                </ul>
